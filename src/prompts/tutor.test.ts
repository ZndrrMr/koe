...
test('neutral conversation is the default without selected context', () => {
  ...
  assert.match(prompt, /Several turns should routinely pass with no praise, correction, modeled answer/i);
...
test('optional context cannot replace the conversation contract', () => {
  ...
  assert.match(prompt, /Preferred Japanese register: teineigo/);
  assert.match(
    prompt,
...
test('representative conversation, confusion, correction, and roleplay cases are included', () => {
  const prompt = tutorSystemPrompt();
  const expectedCases = ['natural-conversation', 'confusion', 'correction-request', 'roleplay-request'];
...
test('natural behavior example sustains conversation without a teaching interruption', () => {
  const natural = CONVERSATION_BEHAVIOR_CASES.find(({ id }) => id === 'natural-conversation');

  assert.ok(natural);
  assert.equal(natural.transcript.match(/Assistant:/g)?.length, 3);
  assert.doesNotMatch(natural.transcript, /正しい|直して|もう一度|練習|try again/i);
});

test('explicit coaching and roleplay requests are honored without becoming defaults', () => {
  const prompt = tutorSystemPrompt();

  assert.match(prompt, /explicitly asks to be taught, translated, corrected, drilled/);
  assert.match(prompt, /Enter a persona or roleplay only when the learner explicitly asks/);
  assert.match(prompt, /then return to the conversation unless they ask to stay in teaching mode/);
});
